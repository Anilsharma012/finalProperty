...
async function testChatFunctionality() {
    ...
    try {
        ...
        const conversationResponse = await fetch(`/api/conversations/find-or-create?propertyId=${propertyId}`, {
            method: 'POST',
            headers: {
        ...
        console.log(`Conversation request: POST /api/conversations/find-or-create?propertyId=${propertyId}`);
        console.log(`Response status: ${conversationResponse.status}`);
        
        if (!conversationResponse.ok) {
            const errorText = await conversationResponse.text();
            console.error(`❌ FAIL: conversation creation`);
            console.error(`URL: /api/conversations/find-or-create?propertyId=${propertyId}`);
        ...
        console.log(`Message request: POST /api/conversations/${conversationId}/messages`);
