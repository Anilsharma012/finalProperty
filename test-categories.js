...
async function testCategoriesModule() {
  ...
  const updatedCategoriesResponse = await fetch(`${baseUrl}/api/admin/categories`, {
    headers: { 'Authorization': `Bearer ${token}` }
  });
  ...
  if (newCount !== initialCount + 1) {
    console.error(`❌ Cache not invalidated. Expected ${initialCount + 1}, got ${newCount}`);
    return false;
  }
  ...
  const updateResponse = await fetch(`${baseUrl}/api/admin/categories/${newCategoryId}`, {
    method: 'PUT',
    headers: { 
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}` 
  ...
  const deleteResponse = await fetch(`${baseUrl}/api/admin/categories/${newCategoryId}`, {
    method: 'DELETE',
    headers: { 'Authorization': `Bearer ${token}` }
  });
  ...
  const finalCategoriesResponse = await fetch(`${baseUrl}/api/admin/categories`, {
    headers: { 'Authorization': `Bearer ${token}` }
  });
  ...
  if (finalCount !== initialCount) {
    console.error(`❌ Final count mismatch. Expected ${initialCount}, got ${finalCount}`);
    return false;
  }
