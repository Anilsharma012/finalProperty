...
function parseCSV(text: string): Record<string,string>[] {
  ...
  for (let i=0;i<text.length;i++){ const ch=text[i], nx=text[i+1];
    if (q){ if (ch==='"' && nx==='"'){ cell+='"'; i++; } else if (ch==='"'){ q=false; }
      else cell+=ch;
    } else { if (ch===','){ row.push(cell.trim()); cell=''; }
      else if (ch==='\n' || ch==='\r'){ if (cell!=='' || row.length){ row.push(cell.trim()); rows.push(row); row=[]; cell=''; }
        if (ch==='\r' && nx==='\n') i++;
      } else if (ch === '"'){ q=true; } else cell+=ch; }
  }
  if (cell!=='' || row.length) { row.push(cell.trim()); rows.push(row); }
  const header = rows.shift()?.map(h=>h.trim()) || [];
  return rows.filter(rw => rw.some(c=>c)).map(rw => Object.fromEntries(header.map((h,i)=>[h, rw[i] ?? ""])));
}
  ...
  try {
    ...
    for (const r of rows){
      try{
        ...
        let cat = await db.collection("os_categories").findOne({ slug: catSlug });
        if (!cat) {
          const catResult = await db.collection("os_categories").insertOne({
            slug: catSlug, 
            name: r.catName || catSlug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()), 
            active: true,
            createdAt: new Date(),
            updatedAt: new Date()
          });
        ...
        let sub = await db.collection("os_subcategories").findOne({ slug: subSlug, category: catSlug });
        if (!sub) {
          const subResult = await db.collection("os_subcategories").insertOne({
            slug: subSlug, 
            name: r.subName || subSlug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()), 
            category: catSlug, 
            active: true,
            createdAt: new Date(),
            updatedAt: new Date()
        ...
        const geo = (r.lat && r.lng) ? { lat: Number(r.lat), lng: Number(r.lng) } : { lat: 0, lng: 0 };
        const payload = {
          category: catSlug,
          subcategory: subSlug,
          name: r.name, 
        ...
        if (existing){ 
          await db.collection("os_listings").updateOne({ _id: existing._id }, { $set: payload }); 
          updated++; 
        } else { 
          await db.collection("os_listings").insertOne({ ...payload, createdAt: new Date() }); 
          created++; 
        }
