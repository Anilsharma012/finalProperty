  ...
  try {
    ...
    const response: ApiResponse<any> = {
      ...
      data: {
        ...
        merchantIdLength: phonePeConfig.merchantId ? phonePeConfig.merchantId.length : 0,
  ...
  try {
    ...
    const paymentMethods = {
      ...
      phonepe: {
        ...
        configured: !!(settings?.payment?.phonePe?.merchantId && settings?.payment?.phonePe?.saltKey),
      },
    };
...
export const testDatabaseConnection: RequestHandler = async (req, res) => {
  try {
    ...
    const settingsCount = await db.collection("admin_settings").countDocuments();
