...
function testStep3() {
    ...
    setTimeout(() => {
        ...
        const enquiryButtons = document.querySelectorAll('[data-testid="enquiry-btn"]');
        ...
        if (enquiryButtons.length > 0) {
            ...
            if (hasEnquiryText) {
                ...
                console.log('⚠️ Warning: Enquiry buttons found but text might be incorrect');
            }
            ...
            console.log('❌ FAIL: No enquiry buttons found with data-testid="enquiry-btn"');
            
            // Debug: Check if old chat buttons still exist
            const chatButtons = document.querySelectorAll('button:contains("Chat"), [class*="chat"]');
            console.log(`🔍 Found ${chatButtons.length} potential chat buttons (should be 0)`);
...
function testEnquirySubmission() {
    ...
    const testEnquiry = {
        ...
        name: 'Test User',
        phone: '+91 9876543210',
        message: 'Hi, I am interested in this property. Please provide more details.',
    ...
    .then(response => {
        ...
        if (response.status === 200 || response.status === 201) {
            ...
            console.log('⚠️ Expected: 400 Bad Request (invalid property ID in test)');
            return response.json();
        } else {
    ...
    .catch(error => {
        ...
        console.log('📋 Expected: This might fail due to CORS or test property ID');
    });
}
