...
export const checkNetworkStatus = async (): Promise<NetworkStatus> => {
  ...
  try {
    ...
    const environment = hostname.includes('.fly.dev') ? 'fly.dev' : 
    ...
    try {
      ...
      diagnostics.push(`Server ping: ${response.status} ${response.statusText}`);
  ...
  return {
    ...
    environment: window.location.hostname.includes('.fly.dev') ? 'fly.dev' : 
...
export const createSafeApiCall = (endpoint: string, options: RequestInit = {}) => {
  return async () => {
    try {
      const controller = new AbortController();
