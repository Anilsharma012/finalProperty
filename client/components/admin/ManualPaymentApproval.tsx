...
export default function ManualPaymentApproval() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/transactions?status=pending,processing', {
        headers: {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      processing: { variant: 'outline' as const, className: 'bg-blue-100 text-blue-800' },
  ...
  const stats = {
    ...
    totalProcessing: transactions.filter(t => t.status === 'processing').length,
    totalAmount: transactions.reduce((sum, t) => sum + t.amount, 0),
    avgAmount: transactions.length ? Math.round(transactions.reduce((sum, t) => sum + t.amount, 0) / transactions.length) : 0,
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-2xl font-bold text-gray-900">Manual Payment Approval</h3>
          <p className="text-gray-600">Review and approve manual payment transactions</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-600">₹{stats.avgAmount}</div>
                ...
                <TableRow key={transaction._id}>
                  ...
                  <TableCell>
                    <div>
                      ...
                      {transaction.propertyTitle && (
                        <p className="text-xs text-gray-500">{transaction.propertyTitle}</p>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center space-x-2 mb-1">
                      ...
                      <span className="capitalize text-sm">{transaction.paymentMethod.replace('_', ' ')}</span>
                    </div>
                    ...
                    {transaction.paymentDetails.upiId && (
                      <p className="text-xs text-gray-500">UPI: {transaction.paymentDetails.upiId}</p>
                    )}
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center space-x-1 text-sm text-gray-500">
                      ...
                      <span>{new Date(transaction.submittedAt).toLocaleDateString()}</span>
                    </div>
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="bg-blue-50 p-4 rounded-lg">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                  ...
                                  <p className="text-xl font-bold text-green-600">₹{selectedTransaction.amount}</p>
                                </div>
                            ...
                            <div className="bg-green-50 p-4 rounded-lg">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                  ...
                                  <p>{new Date(selectedTransaction.submittedAt).toLocaleString()}</p>
                                </div>
                                ...
                                {selectedTransaction.paymentDetails.upiId && (
                                  <div>
                                    ...
                                    <p>{selectedTransaction.paymentDetails.upiId}</p>
                                  </div>
                                )}
                                {selectedTransaction.paymentDetails.reference && (
                                  <div>
                                    <label className="text-sm font-medium text-gray-600">Reference</label>
                                    <p>{selectedTransaction.paymentDetails.reference}</p>
                                  </div>
                                )}
                              </div>
                            </div>
                            ...
                            {selectedTransaction.status === 'pending' && (
                              <div className="flex space-x-3 pt-4">
                                <Button
                                  onClick={() => handleTransactionAction(selectedTransaction._id, 'approve')}
                                ...
                                <Button
                                  onClick={() => handleTransactionAction(selectedTransaction._id, 'reject')}
                                  disabled={actionLoading}
                                  variant="destructive"
                                  className="flex-1"
                                >
