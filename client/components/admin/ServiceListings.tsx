  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (ok && data.success) {
        ...
        setError(getApiErrorMessage(data, status, editingListing ? 'update' : 'create'));
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage individual service provider listings</p>
        </div>
        <Button
      ...
      {showCreateForm && (
        <Card data-testid="service-listing-form">
          <CardHeader>
            <CardTitle>
              {editingListing ? 'Edit Service Listing' : 'Create Service Listing'}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {formData.photos.map((photo, index) => (
                <div key={index}>
                  <label className="block text-sm font-medium mb-2">Photo {index + 1} URL</label>
                  <Input
                    value={photo}
                    onChange={(e) => {
      ...
      {/* Listings Grid */}
      <div data-testid="service-listings-grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {listings.map((listing) => (
          <Card key={listing._id} data-testid={`service-listing-${listing._id}`}>
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="font-semibold text-gray-900">{listing.name}</h3>
                  <p className="text-sm text-gray-500">{listing.category} / {listing.subcategory}</p>
                </div>
                <Badge className={listing.active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
