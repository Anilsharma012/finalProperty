      ...
      try {
        ...
        if (response.ok) {
          addResult(
            "Basic Ping",
            "success",
            `Server responding: ${data.message}`,
            data,
          );
        } else {
          addResult(
      ...
      try {
        ...
        if (response.ok && data.success) {
          addResult(
            "Properties Endpoint",
            "success",
            `Properties API working: ${data.data?.length || 0} properties`,
            data,
          );
        } else {
          addResult(
            "Properties Endpoint",
            "error",
            `Properties API error: ${data.error || "Unknown"}`,
            data,
          );
        }
      } catch (error: any) {
      ...
      try {
        ...
        if (apiResponse.ok && apiResponse.json.success) {
          ...
          addResult(
            "Global API Helper",
            "error",
            `Global API helper error: ${apiResponse.json.error || "Unknown"}`,
            apiResponse.json,
          );
        }
      } catch (error: any) {
        addResult(
          "Global API Helper",
          "error",
          `Global API helper error: ${error.message}`,
        );
      }
        ...
        try {
          ...
          if (apiResponse.ok && apiResponse.json.success) {
            addResult(
              "Property Detail",
              "success",
              `Property loaded: ${apiResponse.json.data.title}`,
            ...
            addResult(
              "Property Detail",
              "error",
              `Property error: ${apiResponse.json.error || "Unknown"}`,
              apiResponse.json,
            );
          }
        } catch (error: any) {
          addResult(
            "Property Detail",
            "error",
            `Property fetch error: ${error.message}`,
          );
        }
      }
