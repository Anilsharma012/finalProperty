...
function checkButtonCount(width) {
  ...
  setTimeout(() => {
    const buttons = document.querySelectorAll('[data-testid="btn-back"],[data-testid="btn-next"],[data-testid="btn-cancel"]');
    ...
    if (buttons.length === 3) {
      ...
      console.log(`✅ ${visibleButtons.length} buttons are visible at ${width}px`);
      
      // Test button functionality
      buttons.forEach((btn, index) => {
        const testId = btn.getAttribute('data-testid');
        console.log(`✅ Button ${index + 1}: ${testId} - ${btn.textContent.trim()}`);
      });
      
    } else {
      console.log(`❌ Expected 3 buttons, found ${buttons.length} at ${width}px`);
    }
...
setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      // Final test
      const allButtons = document.querySelectorAll('[data-testid="btn-back"],[data-testid="btn-next"],[data-testid="btn-cancel"]');
