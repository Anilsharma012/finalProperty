...
function testStep2() {
    ...
    setTimeout(() => {
        ...
        const homeAdCards = document.querySelectorAll('[data-testid="home-ad-card"]');
        ...
        if (homeAdCards.length > 0) {
            ...
            if (firstCard) {
                ...
                console.log(`🖱️ First card clickable: ${isClickable || 'has click handler'}`);
            }
            ...
            console.log('❌ FAIL: No home ad cards found with data-testid="home-ad-card"');
            
            // Debug: Check if PropertyAdsSlider component exists
            const slider = document.querySelector('.property-ads-slider, [class*="PropertyAds"]');
            console.log(`🔍 PropertyAdsSlider component: ${slider ? 'Found' : 'Missing'}`);
            
            // Check if data is loading
            const loadingElements = document.querySelectorAll('.animate-pulse, [class*="loading"]');
